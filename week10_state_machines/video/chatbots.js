...
async function askName(bot, maxAttempts){
    ...
    let fsm = new StateMachine({
        transitions: [
          { name: 'init', from: 'none', to: 'askName' },
          { name: 'hasName',     from: 'askName',  to: 'doubleCheck' },
          { name: 'hasNoName',   from: 'askName', to: 'apologise'  },
          { name: 'answerYes', from: 'doubleCheck', to: 'end'    },
          { name: 'answerNo', from: 'doubleCheck',    to: 'apologise' },
          { name: 'maxAttemptsReached', from: 'apologise',    to: 'end' },
          { name: 'maxAttemptsNotReached', from: 'apologise',    to: 'askName' }
        ],
        methods: {
          onAskName: async function() {
            await speak(bot, 'What is your name?');
            ...
            nAttempts += 1;
            if (userName === ''){
                await speak(bot, 'I did not get your name');
                fsm.hasNoName();
            } else {
                fsm.hasName();
            }
          },
          onDoubleCheck:   async function() {
          ...
          onApologise: async function() {
            await speak(bot, 'Sorry about that');
            if (nAttempts < maxAttempts){
                fsm.maxAttemptsNotReached();
            } else {
                userName = '';
                fsm.maxAttemptsReached();
            }
          },
          onEnd: function() { console.log('End of dialog reached') }
        }
    });

    let fsmPromise = new Promise((resolve) => {
        fsm.observe('onEnd', () => {
            resolve(true);
        });
        fsm.init();
...
//Function to rhyme the user name
